...
test('detects commands', t => {
  ...
  const plugin = r.load(`${__dirname}/_registry/_has-command.js`)
...
test('prevents duplicate commands', t => {
  const r = createRegistry()
  r.load(`${__dirname}/_registry/_duplicate-commands.js`)
  t.is(r.commands.length, 1)
})

test('command names must be non-blank', t => {
  const r = createRegistry()
  r.load(`${__dirname}/_registry/_bad-name-command.js`)
  t.is(r.commands.length, 0)
})

test('commands must be non-blank', t => {
  const r = createRegistry()
  r.load(`${__dirname}/_registry/_missing-function-command.js`)
  t.is(r.commands.length, 0)
})
